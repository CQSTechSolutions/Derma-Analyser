...
import sendEmail, { sendTestEmail, sendEmailWithMailComposer } from '../services/emailService';
            ...
            if (user && user.email) {
              ...
              const emailResponse = await fetch("https://dermaanalyserserver-cqstechsolutions.vercel.app/api/send-report", {
                ...
                body: JSON.stringify({
                  ...
                  reportData: {
                    results: [name, description, symptoms, causes, treatment],
              ...
              if (emailData.success) {
                ...
                Alert.alert(
                  'Success',
                  'Analysis complete and email report sent to your registered email.'
                );
              } else {
                console.error('Email sending failed:', emailData.error);
              }
            } else {
              console.log('User not logged in or no email available for automatic report');
            }
          } catch (emailError) {
            console.error('Failed to send email report:', emailError);
            // Don't show error to user, continue with analysis display
          }
        } catch (error) {
                              ...
                              { 
                                ...
                                onPress: async () => {
                                  try {
                                    ...
                                    console.log('Sending email report to:', user.email);
                                    
                                    // On web platform, skip server test and use mail composer directly
                                    if (Platform.OS === 'web') {
                                      console.log('Web environment detected - using mail composer directly');
                                      const success = await sendEmailWithMailComposer(user.email, emailData);
                                      
                                      if (success) {
                                        Alert.alert(
                                          'Success',
                                          'Email prepared successfully! If you sent it, check your inbox.'
                                        );
                                      } else {
                                        throw new Error('Failed to send email through mail composer');
                                      }
                                    } else {
                                      // Try the unified approach for native platforms
                                      try {
                                        // Try to send test email first to verify server connection
                                        console.log('Sending test email to verify server connection...');
                                        await sendTestEmail(user.email);
                                        console.log('Test email succeeded, proceeding with full report...');
                                      } catch (testError) {
                                        console.warn('Test email failed, but continuing with report:', testError);
                                        // We'll still try the full send process which has its own fallback
                                      }
                                      
                                      // Use the unified email function that tries both backend and mail composer
                                      ...
                                      if (success) {
                                        ...
                                        Alert.alert(
                                          'Success',
                                          'Email report has been sent successfully!'
                                        );
                                      } else {
                                    ...
                                    if (error.message.includes('server')) {
                                      errorMessage = 'Could not connect to email server. Please check if the server is running and try again.';
                                    } else if (error.message.includes('network')) {
                                      errorMessage = 'Network error. Please check your internet connection and try again.';
                                    } else {
                                      errorMessage = error.message || 'Failed to send email. Please try again later.';
                                    }
